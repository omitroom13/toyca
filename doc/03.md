...
RFC 5280 に従っていない気がするが、企業独自の extension で octetstring にされていないデータがついている場合があるようだ(少なくとも microsoft のエンタープライズCAと yubico の attestation key にはあった)。この場合 ASN1:... の書式では設定できず、DER を使うことになる。データの定義や値がわかっていれば以下のような感じで DER にできる。
...
でも同じ extension の値になる。(DER:0410... の 0x0410 はカプセル化によって付加されている。0x04 は octet string データ型, 0x10 はバイト長の意味)
